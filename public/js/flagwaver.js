...
(function (window, document, THREE, undefined) {
  ...
  function Cloth(xSegs, ySegs, restDistance, mass) {
    ...
    for (v = 0; v <= ySegs; v++) {
      for (u = 0; u <= xSegs; u++) {
        particles.push(new Particle(plane(u / xSegs, v / ySegs), mass));
      }
    }
    ...
    if (isDebugMode) {
      window.console.log(
        "FlagWaver: Image properties" +
          ...
          "\n\t" +
          "Canvas size: " +
          window.Math.round(destWidth) +
          "x" +
          window.Math.round(destHeight) +
          "\n\t" +
          "Aspect ratio: " +
  ...
  Flag.prototype.pin = function (edge, spacing) {
    ...
    if (!Util.isNumeric(spacing) || spacing < 1) {
      spacing = 1;
    }
  ...
  Flag.prototype.setTexture = function (texture) {
    if (!(texture instanceof THREE.Texture)) {
      window.console.log("Error: FlagWaver: Invalid texture object.");
      return;
    }
  ...
  Flag.prototype.setTopEdge = function (edge) {
    ...
    switch (edge) {
      case "left":
        this.topEdge = EDGE.Left;
        break;
      case "bottom":
        this.topEdge = EDGE.Bottom;
        break;
      case "right":
        this.topEdge = EDGE.Right;
        break;
      case "top":
      default:
        this.topEdge = EDGE.Top;
  ...
  Flag.prototype.createPublic = function () {
    return new PublicFlag(this);
  };
  ...
  function PublicFlag(flag) {
    ...
    this.options = {
      ...
      get hoisting() {
        return flag.hoisting;
      },
      set hoisting(val) {
        flag.setHoisting(val);
      },
      ...
      set isDefaultSize(val) {
        isDefaultSize = !!val;
        if (isDefaultSize) {
          setDefaultSize(flag.img);
        }
      },

      get width() {
        return flag.options.width;
      },
      set width(val) {
      ...
      get levelOfDetail() {
        return flag.options.levelOfDetail;
      },
  ...
  // Renderer variables
  var vertexShader, fragmentShader, scene, camera, renderer, flag, publicFlag;

  function init() {
    var light, d, poleGeo, poleMat, poleMesh;
  ...
  window.flagWaver = {
    ...
    animation: {
      ...
      step: function () {
        if (animationPaused) animateFrame();
      },
      reset: function () {
        flag.reset();
        render();
